...
class Store:
    ...
    def _binary_search(self, query_embedding, n_candidates=50, project=None, n_results=10,
                       ...
                       kind="all", branch=None, db=None):
        """Two-level binary quantization search over cached vector pools.

        v11 Phase 6b - `embedding_spaces` restricts the candidate pool to
        rows tagged with one of the listed spaces. Pre-v11 rows were
        backfilled to `embedding_space='text'` by migration 021, so the
        filter is safe on legacy data.

        `db` lets a caller on another thread (e.g. the enrichment worker)
        read through its own Connection instead of `self.db` - reading the
        main thread's Connection from a worker thread is a dirty read: an
        uncommitted INSERT on `self.db` is visible here until rollback,
        then vanishes. `self._vector_search` caches vector pools keyed to
        a single Connection, so a caller-supplied `db` bypasses that cache
        via `_binary_search_via_connection` instead.
        """
