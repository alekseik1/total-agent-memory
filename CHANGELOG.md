...
- The validator's path-loss check matches real absolute paths. Its pattern took a slash plus exactly one character and then required another slash, so an absolute path matched only with a one-character segment: `/a/b/c` was checked, `/etc/hosts` and `/Users/alice/project/src/server.py` were not, and a fact merge or a compressed view could drop them unnoticed. A slash starts a path only at line start or after whitespace, a quote, a bracket, `=`, `:`, `,`, `|`, `<`, `>`, `;`, a backtick or `*`, so `EU/Russia` stays prose and a path in `` `code` `` or `**bold**` is still found. A single-segment path counts only with a known file extension (`/README.md`), which keeps slash commands (`/compact`) out; version or date pairs (`python/3.12`, `27.07/28.07`) do not count. A path counts as kept when its text, trailing slash aside, appears anywhere in the new text. Because the check used to match almost nothing, a new compressed view that drops a path is now rejected, so the record gets no `compressed` view (raw and the other views stay), and a fact merge that drops one is refused: on one real store, 12 of 281 compressed views written under the old check fail the new one.
