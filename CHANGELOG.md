...
- The validator's path-loss check matches real absolute paths. Its pattern took a slash plus exactly one character and then required another slash, so an absolute path matched only with a one-character segment: `/a/b/c` was checked, `/etc/hosts` and `/Users/alice/project/src/server.py` were not, and a fact merge or a compressed view could drop them unnoticed. A slash starts a path only at line start or after whitespace, a quote, a bracket, `=`, `:`, `,`, `|`, `<`, `>`, `;`, a backtick or `*`, so `EU/Russia` stays prose and a path in `` `code` `` or `**bold**` is still found; slash commands (`/compact`) and version or date pairs (`python/3.12`, `27.07/28.07`) do not count. A path counts as kept when its text, trailing slash aside, appears anywhere in the new text. Because the check used to match almost nothing, a new compressed view that drops a path is now rejected and the record keeps only `raw`: on one real store, 12 of 281 compressed views written under the old check fail the new one.
